...
from interviewer.agents.interview import InterviewAgent, InterviewDeps, interview_system_prompt
from interviewer.config import LLMConfig, InterviewConfig, LLMProvider, InterviewType, Tone, Difficulty
from interviewer.core import InterviewContext, AgentMessage, AgentResponse, AgentCapability, MessageType
...
class TestInterviewAgentInit:
    ...
    def test_init_anthropic(self, mock_agent_class, mock_anthropic_model):
        ...
        llm_config = LLMConfig(provider=LLMProvider.ANTHROPIC, model="claude-sonnet-4-20250514")
        interview_config = InterviewConfig()
...
class TestInterviewAgentCanHandle:
    ...
    @patch('interviewer.agents.interview.OpenAIModel')
    @patch('interviewer.agents.interview.Agent')
    def test_can_handle_user_message(self, mock_agent_class, mock_openai_model, interview_context, sample_user_message):
    ...
    @patch('interviewer.agents.interview.OpenAIModel')
    @patch('interviewer.agents.interview.Agent')
    def test_can_handle_system_message(self, mock_agent_class, mock_openai_model, interview_context, sample_system_message):
    ...
    @patch('interviewer.agents.interview.OpenAIModel')
    @patch('interviewer.agents.interview.Agent')
    def test_can_handle_other_sender(self, mock_agent_class, mock_openai_model, interview_context):
...
class TestInterviewAgentProcess:
    ...
    @patch('interviewer.agents.interview.OpenAIModel')
    @patch('interviewer.agents.interview.Agent')
    async def test_process_returns_response(self, mock_agent_class, mock_openai_model, interview_context, sample_user_message):
        """Test that process returns a valid AgentResponse."""
        # Setup mock
        mock_result = MagicMock()
        mock_result.output = "That's interesting! Can you tell me more about that experience?"
        ...
        assert "interesting" in response.content.lower() or "tell me more" in response.content.lower()
    
    @pytest.mark.asyncio
    @patch('interviewer.agents.interview.OpenAIModel')
    @patch('interviewer.agents.interview.Agent')
    async def test_process_start_interview(self, mock_agent_class, mock_openai_model, interview_context, sample_system_message):
        """Test processing a start_interview system message."""
        mock_result = MagicMock()
        mock_result.output = "Welcome! Let's begin. Tell me about yourself and your background."
        mock_result.all_messages = MagicMock(return_value=[])
    ...
    @patch('interviewer.agents.interview.OpenAIModel')
    @patch('interviewer.agents.interview.Agent')
    async def test_process_updates_history(self, mock_agent_class, mock_openai_model, interview_context, sample_user_message):
    ...
    @patch('interviewer.agents.interview.OpenAIModel')
    @patch('interviewer.agents.interview.Agent')
    async def test_process_handles_error(self, mock_agent_class, mock_openai_model, interview_context, sample_user_message):
...
class TestInterviewAgentContext:
    ...
    @patch('interviewer.agents.interview.OpenAIModel')
    @patch('interviewer.agents.interview.Agent')
    async def test_context_includes_company_role(self, mock_agent_class, mock_openai_model, interview_context, sample_system_message):
...
class TestInterviewAgentLive:
    ...
    @pytest.mark.asyncio
    async def test_live_conversation_flow(self, interview_context, sample_system_message):
