...
class SummaryAgent(BaseInterviewAgent):
    ...
    def can_handle(self, message: AgentMessage, context: InterviewContext) -> float:
        ...
        if any(keyword in content for keyword in [
            "summary", "wrap up", "conclude", "end interview", "final thoughts"
        ]):
            return 0.9
    ...
    def _generate_comprehensive_summary(self, context: InterviewContext) -> Dict[str, Any]:
        ...
        summary_data = {
            ...
            "detailed_analysis": {
                ...
                "areas_for_improvement": self._identify_improvement_areas(context, performance_scores),
            ...
            "conversation_insights": {
                "average_response_length": self._calculate_avg_response_length(user_turns),
    ...
    def _extract_performance_scores(self, feedback_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _analyze_conversation_flow(self, context: InterviewContext) -> Dict[str, Any]:
        ...
        engagement_score = min(1.0, avg_response_length / 30.0)  # 30 words = good engagement
        ...
        for turn in user_turns[-5:]:  # Last 5 responses
            ...
            confident_count = sum(1 for indicator in confidence_indicators if indicator in content_lower)
            uncertain_count = sum(1 for indicator in uncertainty_indicators if indicator in content_lower)
    ...
    def _assess_technical_competency(self, context: InterviewContext) -> Dict[str, Any]:
        ...
        technical_terms = [
            "algorithm", "model", "data", "python", "sql", "machine learning",
            "statistics", "analysis", "database", "api", "framework", "library",
            "optimization", "performance", "scalability", "architecture"
        ]
        
        technical_mentions = sum(1 for term in technical_terms if term in all_user_content)
    ...
    def _assess_communication_skills(self, context: InterviewContext) -> Dict[str, Any]:
        ...
        structure_indicators = ["first", "second", "then", "because", "therefore", "for example"]
        structure_count = 0
        ...
        structure_score = min(1.0, structure_count / (len(user_turns) * 2))  # 2 indicators per response ideal
    ...
    def _identify_phases_covered(self, context: InterviewContext) -> List[str]:
        ...
        all_content = " ".join(turn.content for turn in context.conversation_history).lower()
        
        if any(keyword in all_content for keyword in ["experience", "background", "previous"]):
            phases.append("Background Discussion")
        
        if any(keyword in all_content for keyword in ["technical", "algorithm", "code"]):
    ...
    def _identify_key_strengths(self, context: InterviewContext, performance_scores: Dict[str, Any]) -> List[str]:
    ...
    def _identify_improvement_areas(self, context: InterviewContext, performance_scores: Dict[str, Any]) -> List[str]:
    ...
    def _generate_recommendations(
        ...
        if overall_score >= 0.8:
            recommendations.append("Excellent interview performance! Focus on maintaining this level.")
        elif overall_score >= 0.6:
            recommendations.append("Good foundation demonstrated. Continue practicing interview skills.")
        else:
            recommendations.append("Focus on providing more detailed and specific responses.")
        ...
        if communication_analysis["score"] < 0.6:
            recommendations.append("Practice structuring responses with clear examples and logical flow.")
    ...
    def _analyze_topic_coverage(self, context: InterviewContext) -> List[str]:
        ...
        topic_keywords = {
            ...
            "Data Analysis": ["data analysis", "statistics", "visualization", "insights"],
            "Programming": ["python", "code", "programming", "implementation", "development"],
            "Databases": ["sql", "database", "query", "data storage"],
            "Business Understanding": ["business", "stakeholder", "requirement", "process"],
            "Problem Solving": ["approach", "solution", "problem", "challenge", "methodology"]
        }
