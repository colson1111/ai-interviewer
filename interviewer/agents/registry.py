...
from ..core import AgentCapability, InterviewContext, AgentMessage
...
class AgentRegistry:
    ...
    def register_agent(
        ...
        print(f"Registered agent: {agent.name} with capabilities: {[c.value for c in agent.get_capabilities()]}")
    ...
    def get_capabilities_summary(self) -> Dict[str, List[str]]:
        ...
        for capability, agent_names in self._capability_index.items():
            enabled_agents = [name for name in agent_names 
                            if name in self._agents and self._agents[name].agent.is_enabled]
            if enabled_agents:
    ...
    def get_registry_status(self) -> Dict[str, Any]:
        ...
        enabled_agents = len([reg for reg in self._agents.values() if reg.agent.is_enabled])
