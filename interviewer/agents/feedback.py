...
class FeedbackAgent(BaseInterviewAgent):
    ...
    def _analyze_response(
        self, response: str, context: InterviewContext
    ) -> Dict[str, Any]:
        ...
        overall_score = self.contextual.calculate_contextual_score(
            metrics, response_type
        )

        # Generate contextual feedback
        strengths = self.contextual.generate_contextual_strengths(
            metrics, response_type
        )
        improvements = self.contextual.generate_contextual_improvements(
            metrics, response_type
        )
        suggestions = self.contextual.generate_contextual_suggestions(
            metrics, response_type
        )
        technical_insights = self.contextual.generate_contextual_technical_insights(
            metrics, response_type
        )
        communication_tips = self.contextual.generate_contextual_communication_tips(
            metrics, response_type
        )

        # Get contextual display metrics
        display_metrics = self.contextual.get_contextual_display_metrics(
            metrics, response_type
        )
