...
class DocumentParser:
    ...
    def extract_key_info(text: str, doc_type: str = "document") -> dict:
        ...
        if doc_type == "resume":
            ...
            sections = {
                "experience": any("experience" in line or "work" in line for line in lines),
                "education": any("education" in line or "degree" in line for line in lines),
                "skills": any("skills" in line or "technical" in line for line in lines),
                "contact": any("email" in line or "@" in line or "phone" in line for line in lines)
            }
            ...
            companies = []
            for line in lines:
                if any(keyword in line for keyword in ["inc", "corp", "llc", "ltd", "company", "technologies"]):
            ...
            sections = {
                "requirements": any("requirements" in line or "qualifications" in line for line in lines),
                "responsibilities": any("responsibilities" in line or "duties" in line for line in lines),
                "benefits": any("benefits" in line or "perks" in line for line in lines),
                "salary": any("salary" in line or "$" in text or "compensation" in line for line in lines)
            }
            info["sections"] = sections
            
            # Extract potential skills/technologies mentioned
            tech_keywords = ["python", "javascript", "sql", "aws", "docker", "kubernetes", "react", "angular", "machine learning", "ai"]
...
def create_document_context(resume_text: Optional[str], job_desc_text: Optional[str]) -> str:
    """
