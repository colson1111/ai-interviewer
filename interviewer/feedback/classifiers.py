...
class ResponseClassifier:
    ...
    def classify_response_type(self, response: str, context: Any = None) -> str:
        ...
        if context and hasattr(context, "interview_config"):
            ...
            elif interview_type == "behavioral" and primary_type in [
                "behavioral",
                "leadership",
            ]:
                scores["behavioral"] *= 1.3
                scores["leadership"] *= 1.2
            elif interview_type == "case_study" and primary_type in [
                "project",
                "technical",
            ]:
                scores["project"] *= 1.3
    ...
    def get_response_subtype(self, response: str, response_type: str) -> str:
        ...
        if response_type == "technical":
            ...
            if any(term in response_lower for term in ["sql", "database", "query"]):
                ...
                term in response_lower for term in ["pandas", "numpy", "dataframe"]
            ):
                return "data_analysis"
            elif any(
                term in response_lower for term in ["algorithm", "complexity", "big o"]
            ):
                return "algorithms"
            elif any(
                term in response_lower for term in ["machine learning", "ml", "model"]
            ):
            ...
            if any(
                ...
                term in response_lower for term in ["project", "initiative", "campaign"]
            ):
                return "project_experience"
            elif any(
                term in response_lower
                for term in ["challenge", "problem", "difficulty"]
            ):
                return "challenge_handling"
            elif any(
                term in response_lower
                for term in ["team", "collaboration", "stakeholder"]
            ):
                return "team_work"
            else:
            ...
            if any(term in response_lower for term in ["led", "managed", "supervised"]):
                ...
                term in response_lower for term in ["strategy", "vision", "planning"]
            ):
                return "strategic_thinking"
            elif any(
                term in response_lower for term in ["mentored", "coached", "guided"]
            ):
                return "mentoring"
            elif any(
                term in response_lower
                for term in ["influence", "stakeholder", "presentation"]
            ):
            ...
            if any(
                term in response_lower
                for term in ["implementation", "deployment", "launch"]
            ):
                return "project_execution"
            elif any(
                term in response_lower for term in ["timeline", "budget", "scope"]
            ):
                return "project_management"
            elif any(
                term in response_lower
                for term in ["requirement", "specification", "design"]
            ):
