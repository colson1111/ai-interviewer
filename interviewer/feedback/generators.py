...
class FeedbackGenerator:
    ...
    def generate_strengths(
        self, metrics: AssessmentMetrics, response_type: str
    ) -> List[str]:
    ...
    def generate_feedback_display(
        ...
        type_indicator = (
            f" | Type: {response_type.title()}" if response_type != "general" else ""
        )

        # Strengths
        strengths = self.generate_strengths(metrics, response_type)
        strengths_text = (
            ", ".join(strengths[:2]) if strengths else "Good response structure"
        )

        # Improvements
        improvements = self.generate_improvements(metrics, response_type)
        improvements_text = (
            ", ".join(improvements[:2]) if improvements else "Continue building on this"
        )
        ...
        insights_text = (
            ...
            if insights and response_type == "technical"
            else ""
        )
    ...
    def generate_summary_feedback(
        self, all_metrics: List[AssessmentMetrics], response_types: List[str]
    ) -> Dict[str, Any]:
        ...
        avg_technical_depth = (
            sum(m.technical_depth for m in all_metrics) / total_responses
        )
        avg_communication_clarity = (
            sum(m.communication_clarity for m in all_metrics) / total_responses
        )
