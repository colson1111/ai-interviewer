...
class ResponseAssessor:
    ...
    def assess_communication_clarity(self, response: str) -> float:
        ...
        complexity_score = max(
            0.0, 1.0 - (avg_sentence_length - 15) / 10.0
        )  # Prefer 15 words per sentence

        return round(
            (structure_score * 0.4 + clarity_score * 0.4 + complexity_score * 0.2), 2
        )
    ...
    def assess_structure(self, response: str) -> float:
        ...
        flow_count = sum(
            1 for indicator in flow_indicators if indicator.lower() in response.lower()
        )
        ...
        topic_sentences = sum(
            ...
            and any(
                ...
                for word in ["was", "is", "had", "did", "created", "developed"]
            )
        )
    ...
    def assess_technical_accuracy(self, response: str) -> float:
        ...
        precise_count = sum(
            1 for term in precise_terms if term.lower() in response.lower()
        )
    ...
    def assess_problem_solving(self, response: str) -> float:
        ...
        return round(
            (systematic_score * 0.4 + analysis_score * 0.3 + solution_score * 0.3), 2
        )
    ...
    def assess_communication_style(self, response: str) -> float:
        ...
        inappropriate_count = sum(
            1 for word in inappropriate_words if word.lower() in response.lower()
        )
        ...
        balanced_count = sum(
            1
            for indicator in balanced_indicators
            if indicator.lower() in response.lower()
        )
        ...
        return round(
            max(
                0.0,
                (professional_score * 0.5 + balanced_score * 0.5)
    ...
    def assess_impact_quantification(self, response: str) -> float:
        ...
        metrics = re.findall(
            r"\d+%|\d+ percent|\d+ times|\d+x|\$\d+|\d+ dollars",
            response,
            re.IGNORECASE,
        )
        ...
        return round(
            (metric_score * 0.5 + impact_score * 0.3 + comparison_score * 0.2), 2
        )
    ...
    def assess_leadership_demonstration(self, response: str) -> float:
        ...
        action_count = sum(
            1 for action in leadership_actions if action.lower() in response.lower()
        )
        ...
        team_count = sum(
            1 for indicator in team_indicators if indicator.lower() in response.lower()
        )
    ...
    def extract_keywords(self, response: str) -> List[str]:
        ...
        all_keywords = (
            self.technical_keywords + self.star_keywords + self.leadership_keywords
        )
    ...
    def assess_sentiment(self, response: str) -> str:
        ...
        positive_count = sum(
            1 for word in positive_words if word.lower() in response.lower()
        )
        negative_count = sum(
            1 for word in negative_words if word.lower() in response.lower()
        )
    ...
    def assess_response(self, response: str, context: Any = None) -> AssessmentMetrics:
        ...
        metrics.leadership_demonstration = self.assess_leadership_demonstration(
            response
        )
