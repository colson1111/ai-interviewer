...
class AgentSelector:
    ...
    def select_agents(self, message: 'AgentMessage', context: 'InterviewContext') -> RoutingDecision:
    ...
    def _calculate_agent_scores(self, message: 'AgentMessage', context: 'InterviewContext') -> Dict[str, float]:
        ...
        if has_company_mention:
            ...
            if len(message.content.split()) > 100:
                ...
                scores["interview"] = 0.9  # Much higher interview score for detailed responses
                print(f"########## ROUTING: Company mention in detailed response - prioritizing interview")
            else:
                scores["search"] = 0.4  # Lower search score generally
                scores["interview"] = 0.7  # Higher interview score
                print(f"########## ROUTING: Company mention detected - minimal search trigger")
        ...
        has_leadership_mention = any(role in content_lower for role in leadership_indicators)

        if has_leadership_mention:
            scores["search"] = 0.3  # Lower search score
            scores["interview"] = 0.8  # Higher interview score
            print(f"########## ROUTING: Leadership mention detected - minimal search trigger")
        ...
        if any(tech in content_lower for tech in tech_keywords):
            ...
            print(f"########## ROUTING: Technology mention detected - minimal search trigger")
        ...
        if any(indicator in content_lower for indicator in project_indicators):
            ...
            scores["interview"] = 0.8  # High interview score
            print(f"########## ROUTING: Project/role mention detected - minimal search trigger")
        ...
        if any(time_indicator in content_lower for time_indicator in time_indicators):
            scores["search"] = 0.2  # Very low search score
            scores["interview"] = 0.8  # High interview score
            print(f"########## ROUTING: Time-based mention detected - minimal search trigger")
        ...
        if any(entity in content_lower for entity in specific_entities):
            scores["search"] = 0.2  # Very low search score
            scores["interview"] = 0.8  # High interview score
            print(f"########## ROUTING: Specific entity mention detected - minimal search trigger")
        ...
        if scores["search"] > 0.0:
            print(f"########## ROUTING: Final scores - search: {scores['search']}, interview: {scores['interview']}")
